...
namespace DiscountStoreTests.Modules.Checkout.Services
{
    ...
    public class ItemsManipulationServiceTests
    {
        ...
        public void GivenSingleItem_ReturnsCollectionWithThatItem()
        {
            var itemsToMerge = new List<ItemViewModel>
            {
                new ItemViewModel(new ProductViewModel("Product1", 1.0, 1), 1, null)
            };
            var sut = new ItemsManipulationService();
        ...
        public void GivenTwoItemsWithDiffrentProductId_ReturnsCollectionWithTheseItems()
        {
            ...
            {
                new ItemViewModel(new ProductViewModel("Product1", 1.0, 1), 1, null),
                new ItemViewModel(new ProductViewModel("Product1", 1.0, 2), 1, null)
            };
            var sut = new ItemsManipulationService();
            ...
            {
                Assert.Equal(1, item1.Product.Id);
            },item2 =>
            {
                Assert.Equal(2, item2.Product.Id);
            });
        }
        ...
        public void GivenTwoItemsWithSameProductIdButOneWithDiscount_ReturnsCollectionWithTheseItems()
        {
            ...
            {
                new ItemViewModel(new ProductViewModel("Product1", 1.0, 1), 1, null),
                new ItemViewModel(new ProductViewModel("Product1", 1.0, 1), 1, new DiscountViewModel(DiscountType.TwoForX, 0.5))
            };
            var sut = new ItemsManipulationService();
            ...
            {
                Assert.Equal(1, item2.Product.Id);
            });
        }
        
        [Fact]
        public void GivenTwoItemsWithSameProductIdAndDiscountAndOneWithoutDiscount_ReturnsCollectionWithMergedDiscountedItemsAndNotMergedItemWithoutDiscountTheseItems()
        {
            var itemsToMerge = new List<ItemViewModel>
            {
                new ItemViewModel(new ProductViewModel("Product1", 1.0, 1), 1, null),
